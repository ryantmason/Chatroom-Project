...
messageFlags buildMessage(messageDataType *message, char *name, char *note)
{
    ...
    if( strncmp(message->messageNote , "JOIN", 4) == STR_EQ )
        {
            message->chatFlag = JOIN;
        }
        else if( strncmp(message->messageNote , "LEAVE", 5) == STR_EQ )
        {
            message->chatFlag = LEAVE;
        }
        else if( strncmp(message->messageNote , "SHUTDOWN ALL", 12) == STR_EQ )
        {
            message->chatFlag = SHUTDOWN_ALL;
        }
        else if( strncmp(message->messageNote , "SHUTDOWN", 8) == STR_EQ )
        {
